...
__global__ void calc_best_score(char *d_main_seq, char *d_sec_seq,
		int *d_hash_con, int *d_hash_semi_con, weight_t *weight, int offset,
		float *results, int res_block_len) {
	...
	for (int hyphen = 1; hyphen <= res_block_len; hyphen++) {
		...
		} else if (d_hash_con[hash(d_main_seq[tid + offset],
				d_sec_seq[tid + delta])]) {
			results[tid + (hyphen - 1) * res_block_len] = -weight->w2;
		} else if (d_hash_semi_con[hash(d_main_seq[tid + offset],
				d_sec_seq[tid + delta])]) {
